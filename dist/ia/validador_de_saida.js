...
exports.relatoPadrao = relatoPadrao;
exports.validarRelato = validarRelato;
exports.mesclarRelatos = mesclarRelatos;
...
function asString(valor, padrao = 'nao_informado') {
    if (typeof valor === 'string' && valor.trim())
        return valor.trim();
    return padrao;
}
function asSimNao(valor) {
    if (valor === true || valor === 'sim')
        return 'sim';
    if (valor === false || valor === 'nao' || valor === 'não')
        return 'nao';
    if (tipos_1.VALORES_SIM_NAO.includes(valor))
        return valor;
    return 'nao_informado';
}
function asFlag(valor) {
...
function relatoPadrao(parcial = {}) {
    return { ...tipos_1.RELATO_VAZIO, ...parcial };
}
function validarRelato(entrada) {
    if (!entrada || typeof entrada !== 'object') {
        return { ok: false, relato: tipos_1.RELATO_VAZIO };
    }
    const bruto = entrada;
    const idade = tipos_1.IDADE_GRUPOS.includes(bruto.idade_grupo)
        ? bruto.idade_grupo
        : 'nao_informado';
    const risco = tipos_1.RISCOS_MENTAIS.includes(bruto.risco_mental)
        ? bruto.risco_mental
        : 'nao_mencionado';
    const relato = {
    ...
    const formatoOk = Array.isArray(bruto.sintomas) &&
        tipos_1.IDADE_GRUPOS.includes(bruto.idade_grupo) &&
        tipos_1.VALORES_SIM_NAO.includes(asSimNao(bruto.gestante));
 ...
 */
function preferirComNegacao(atual, novo, vazio, negacao) {
 ...
 */
function preferirFlagComNegacao(atual, novo) {
...
function mesclarRelatos(base, extra) {
    const unirArrays = (a = [], b = []) => [...new Set([...a, ...b])];
    return {
